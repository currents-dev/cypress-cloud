...
export const createProgram = (command: Command = new Command()) =>
  command
    .name("estefafdez-cloud")
    .description(
      `
Run Cypress tests on CI using
...
${getLegalNotice()}
    ...
    .addOption(
      new Option(
        ...
        "Specify the config file for estefafdez-cloud, defaults to 'estefafdez.config.js' and will be searched in the project root, unless an aboslue path is provided"
      ).default(undefined)
    )
    .addOption(
      new Option(
        `--cloud-debug [true | string]`,
        `Enable debug mode for estefafdez-cloud, this will print out logs for troubleshooting. Values: [true | ${Object.values(
          DebugMode
        ).join(
          " | "
